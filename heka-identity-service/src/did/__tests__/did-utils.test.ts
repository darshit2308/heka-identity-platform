...
describe('DID Utils: Deterministic Key Selector', () => {
  ...
  it('should not depend on array ordering — same result regardless of verificationMethod order', () => {
    const documentWithKeyFirst = {
      ...
      verificationMethod: [
        {
          id: 'did:example:abc#assertion-key',
          type: 'Ed25519VerificationKey2020',
          controller: 'did:example:abc',
          publicKeyMultibase: 'zCorrectKey',
        },
        {
          id: 'did:example:abc#authentication-key',
          type: 'Ed25519VerificationKey2020',
          controller: 'did:example:abc',
  ...
  it('should use exact fragment matching, not substring — #backup-assertion-key must NOT match assertion-key', () => {
    const mockDidDocument = {
      id: 'did:example:123',
      verificationMethod: [
        {
          id: 'did:example:123#backup-assertion-key',
          type: 'Ed25519VerificationKey2020',
          controller: 'did:example:123',
          publicKeyMultibase: 'zWrongKey',
        },
  ...
  it('should throw an error if the verification method array is missing or malformed', () => {
    ...
    expect(() => getDeterministicSigningKey({ verificationMethod: null } as any, 'assertion-key')).toThrow(
...
describe('DID Utils: DID Resolution', () => {
  ...
  it('should throw if the resolver returns null for an unknown DID', async () => {
    ...
    await expect(resolveDidDocument('did:hedera:testnet:unknownDID', mockResolver)).rejects.toThrow(
      'DID resolution failed: document not found for did:hedera:testnet:unknownDID',
    )
  })

  it('should throw if the resolved document id does not match the requested DID', async () => {
    const mismatchedDocument = {
      ...mockHederaDidDocument,
      id: 'did:hedera:testnet:completelyDifferentDID',
    }
    const mockResolver: DidResolverFn = vi.fn().mockResolvedValue(mismatchedDocument)

    await expect(
      resolveDidDocument('did:hedera:testnet:z6MkhaXgBZDvotDkL5257faiztiuC2ZXOS', mockResolver),
    ).rejects.toThrow(
      'DID resolution mismatch: requested did:hedera:testnet:z6MkhaXgBZDvotDkL5257faiztiuC2ZXOS but resolver returned did:hedera:testnet:completelyDifferentDID',
    )
  })
